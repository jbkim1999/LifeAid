...
public class Teleporter : MonoBehaviour
{
    ...
    private void UpdatePath()
    {
        ...
        velocity = Quaternion.AngleAxis(-angle, bodyTransform.right) * bodyTransform.forward * strength;
        RaycastHit hit;
        Vector3 pos = bodyTransform.position; // take off position
        vertexList.Add(pos);

        while (!groundDetected && vertexList.Count < maxVertexcount)
        {
            Vector3 newPos = pos + OVRInput.GetLocalControllerRotation(Controller) * new Vector3(-0.2f, 0f, 0.6f) + velocity * vertexDelta + 0.5f * Physics.gravity * vertexDelta * vertexDelta;
